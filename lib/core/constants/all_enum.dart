...
enum TimeDuration {
  blitz({
    "1-0": TimeSetting("1 min", 1, 0),
    "1-1": TimeSetting("1 | 1", 1, 1),
    "2-1": TimeSetting("2 | 1", 2, 1),
  }),
  rapid({
    "3-0": TimeSetting("3 min", 3, 0),
    "3-2": TimeSetting("3 | 2", 3, 2),
    "5-0": TimeSetting("5 min", 5, 0),
  }),
  normal({
    "10-0": TimeSetting("10 min", 10, 0),
    "15-10": TimeSetting("15 | 10", 15, 10),
    "30-0": TimeSetting("30 min", 30, 0),
  });

  final Map<String, TimeSetting> options;
  const TimeDuration(this.options);

  static final Map<Map<String, TimeSetting>, TimeDuration> _fromValue = {
    for (var time in values) time.options: time,
  };

  static final Map<String, TimeDuration> _fromName = {
    for (var time in values) time.name: time,
  };

  static TimeDuration? fromValue(Map<String, TimeSetting> value) =>
      _fromValue[value];

  static TimeDuration? fromName(String name) => _fromName[name];
}
...
enum GameResultType { win, lose, draw, ongoing }
